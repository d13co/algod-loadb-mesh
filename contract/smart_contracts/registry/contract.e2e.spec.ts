...
const PUT = 'put(byte[16],byte[],byte[],byte[],byte[])void'
const REMOVE = 'remove(byte[16])void'
...
/** Stand-in for the off-chain keyed hash of a node id: the contract only sees 16 bytes. */
const tag = (label: string) => new Uint8Array(createHash('sha256').update(label).digest()).subarray(0, TAG_LEN)
...
describe('registry contract', () => {
  ...
  /** Records by hex tag. */
  const records = async (client: RegistryClient) => new Map([...(await client.state.box.records.getMap())].map(([k, v]) => [hex(k), v]))
  ...
  describe('interface', () => {
    test('exposes put and remove as ARC-4 methods, and bare create, update and delete', () => {
      ...
      expect(APP_SPEC.state.maps.box.records).toMatchObject({ keyType: 'AVMBytes', valueType: 'AVMBytes', prefix: Buffer.from('n').toString('base64') })
      expect(hex(selector(PUT))).toBe('324f5a1b')
      expect(hex(selector(REMOVE))).toBe('12ad14c2')
    })
  })
  ...
  describe('creation', () => {
    ...
    test('a method call cannot create', async () => {
      ...
      const { approval, clear } = programs()
      await expect(
        algorand.send.appCreate({ sender: testAccount, approvalProgram: approval, clearStateProgram: clear, args: [selector(REMOVE), tag('k44')] }),
      ).rejects.toThrow(/logic eval error/)
    })
  })

  describe('put', () => {
    test('stores the value verbatim in box n<tag>', async () => {
      const { algorand, client } = await setup()
      const value = bytes(500)

      await client.send.put(putParams(tag('k44'), value))

      expect(await algorand.app.getBoxValue(client.appId, boxKey(tag('k44')))).toEqual(value)
      expect(await records(client)).toEqual(new Map([[hex(tag('k44')), value]]))
    })
    ...
    test('is a full replace, growing and shrinking the box', async () => {
      ...
      for (const len of [300, 1900, 29, MAX_PART_LEN, MAX_PART_LEN + 1, 16000, 2048, 2049, 600]) {
        ...
        await client.send.put(putParams(tag('k44'), value))
        expect((await records(client)).get(hex(tag('k44')))).toEqual(value)
      }
    })
    ...
    test('concatenates parts of any size in order', async () => {
      ...
      await client.send.put(putParams(tag('k44'), [value.subarray(0, cut[0]), value.subarray(cut[0], cut[1]), value.subarray(cut[1], cut[2]), value.subarray(cut[2])]))
      expect((await records(client)).get(hex(tag('k44')))).toEqual(value)

      // empty parts contribute nothing, wherever they are
      const empty = new Uint8Array()
      await client.send.put(putParams(tag('k44'), [empty, value.subarray(0, MAX_PART_LEN), empty, value.subarray(MAX_PART_LEN)]))
      expect((await records(client)).get(hex(tag('k44')))).toEqual(value)
    })

    test('accepts an empty value', async () => {
      const { client } = await setup()
      await client.send.put(putParams(tag('empty'), new Uint8Array()))
      expect(await records(client)).toEqual(new Map([[hex(tag('empty')), new Uint8Array()]]))
    })

    test('keeps records independent', async () => {
      const { client } = await setup()
      await client.send.put(putParams(tag('a'), bytes(10, 1)))
      await client.send.put(putParams(tag('b'), bytes(20, 2)))
      await client.send.remove(removeParams(tag('a')))

      expect(await records(client)).toEqual(new Map([[hex(tag('b')), bytes(20, 2)]]))
    })
  })
  ...
  describe('remove', () => {
    test('removes the box', async () => {
      const { algorand, client } = await setup()
      await client.send.put(putParams(tag('k44'), bytes(100)))
      await client.send.remove(removeParams(tag('k44')))
      expect(await algorand.app.getBoxNames(client.appId)).toEqual([])
    })

    test('is idempotent', async () => {
      const { client } = await setup()
      await client.send.remove(removeParams(tag('never-written')))
      await client.send.remove(removeParams(tag('never-written')))
    })
  })

  describe('rejections', () => {
    const raw = (appId: bigint, sender: Address, args: Uint8Array[]) => ({ sender, appId, args, boxReferences: boxRefs(tag('k44')), populateAppCallResources: false })

    test('unknown method selector', async () => {
      const { algorand, creator, appId } = await setup()
      await expect(algorand.send.appCall(raw(appId, creator, [selector('get(string)byte[]'), tag('k44')]))).rejects.toThrow(/logic eval error/)
    })

    test('the former raw interface', async () => {
      const { algorand, creator, appId } = await setup()
      await expect(algorand.send.appCall(raw(appId, creator, [enc('put'), boxKey(tag('k44')), bytes(10)]))).rejects.toThrow(/logic eval error/)
      await expect(algorand.send.appCall(raw(appId, creator, [enc('del'), boxKey(tag('k44'))]))).rejects.toThrow(/logic eval error/)
    })

    test('tags of another length', async () => {
      const { algorand, creator, appId } = await setup()
      const empty = abiBytes(new Uint8Array())
      for (const bad of [tag('k44').subarray(0, TAG_LEN - 1), Uint8Array.from([...tag('k44'), 0]), enc('k44')]) {
        await expect(algorand.send.appCall(raw(appId, creator, [selector(PUT), bad, abiBytes(bytes(10)), empty, empty, empty]))).rejects.toThrow(
          /invalid number of bytes/,
        )
        await expect(algorand.send.appCall(raw(appId, creator, [selector(REMOVE), bad]))).rejects.toThrow(/invalid number of bytes/)
      }
    ...
    test('put with missing parts', async () => {
      const { algorand, creator, appId } = await setup()
      await expect(algorand.send.appCall(raw(appId, creator, [selector(PUT), tag('k44'), abiBytes(bytes(10))]))).rejects.toThrow(/logic eval error/)
    })

    test('parts whose ABI length prefix does not match', async () => {
      const { algorand, creator, appId } = await setup()
      const part = abiBytes(bytes(10))
      const truncated = part.subarray(0, part.length - 1)
      const empty = abiBytes(new Uint8Array())
      await expect(algorand.send.appCall(raw(appId, creator, [selector(PUT), tag('k44'), truncated, empty, empty, empty]))).rejects.toThrow(/invalid number of bytes/)
      expect(await algorand.app.getBoxNames(appId)).toEqual([])
    })
    ...
    test('methods with another on-completion', async () => {
      const { algorand, creator, appId } = await setup()
      const empty = abiBytes(new Uint8Array())
      const put = [selector(PUT), tag('k44'), abiBytes(bytes(10)), empty, empty, empty]
    ...
    describe('non-creator', () => {
      test('put', async () => {
        ...
        await expect(s.client.send.put(putParams(tag('k44'), bytes(10)))).rejects.toThrow(/creator only/)
        expect((await records(client)).size).toBe(0)
      })

      test('remove', async () => {
        const { client } = await setup()
        await client.send.put(putParams(tag('k44'), bytes(10)))
        const s = await stranger(client)
        await expect(s.client.send.remove(removeParams(tag('k44')))).rejects.toThrow(/creator only/)
        expect((await records(client)).get(hex(tag('k44')))).toEqual(bytes(10))
      })
  ...
  describe('minimum balance', () => {
    ...
    test('put fails until the app account is funded', async () => {
      ...
      await expect(client.send.put(putParams(tag('k44'), value))).rejects.toThrow(/balance .* below min/)
      ...
      const required = info.minBalance.microAlgo + boxMbr(boxKey(tag('k44')).length, value.length) + 100_000n
  ...
  describe('transaction limits (consensus v42)', () => {
    test('args of put are selector, tag and four parts, each part with a 2-byte length prefix', async () => {
      const { client } = await setup()
      const result = await client.send.put(putParams(tag('k44'), bytes(10)))
      const args = result.transaction.applicationCall!.appArgs
      expect(args.map((a) => a.length)).toEqual([4, TAG_LEN, 2 + 10, 2, 2, 2])
      expect(args[1]).toEqual(tag('k44'))
      expect(args.reduce((n, a) => n + a.length, 0)).toBe(putArgsLen(10))
    })

    test(`the largest value is ${MAX_APP_ARGS_LEN} - ${putArgsLen(0)}, whatever the node id`, async () => {
      const { client } = await setup()
      const max = MAX_VALUE_LEN
      expect(max).toBe(16356)

      await client.send.put(putParams(tag('k44'), bytes(max)))
      expect((await records(client)).get(hex(tag('k44')))?.length).toBe(max)

      await expect(client.send.put(putParams(tag('k44'), bytes(max + 1)))).rejects.toThrow(/total length is too long/)
    })

    test(`a part is limited to ${MAX_PART_LEN} bytes, one arg less its length prefix`, async () => {
      const { client } = await setup()
      const empty = new Uint8Array()
      await client.send.put(putParams(tag('k44'), [bytes(MAX_PART_LEN), empty, empty, empty]))
      await expect(client.send.put(putParams(tag('k44'), [bytes(MAX_PART_LEN + 1), empty, empty, empty]))).rejects.toThrow(/length is too long/)
    })

    test('arg bytes beyond 2048 cost 0.0001 min fee each, rounded up', async () => {
      const { client } = await setup()
      for (const argsLen of [2048, 2049, 3048, 16384]) {
        const value = bytes(argsLen - putArgsLen(0))
        const fee = callFee(MIN_FEE, argsLen)
        await expect(client.send.put(putParams(tag('k44'), value, { fee: fee - 1n }))).rejects.toThrow(/fees is less than/)
        await client.send.put(putParams(tag('k44'), value, { fee }))
      }
    ...
    test(`${BOX_REFS} box references cover any put and remove`, async () => {
      ...
      const max = MAX_VALUE_LEN
      await client.send.put(putParams(tag('k44'), bytes(max)))
      await client.send.put(putParams(tag('k44'), bytes(1)))
      await client.send.put(putParams(tag('k44'), bytes(max)))
      await client.send.remove(removeParams(tag('k44')))
    })
    ...
    test(`one reference budgets ${BOX_REF_BUDGET} bytes of box value, excluding the key`, async () => {
      const { client } = await setup()
      await client.send.put(putParams(tag('k44'), bytes(BOX_REF_BUDGET), { refs: 1 }))
      await expect(client.send.put(putParams(tag('k44'), bytes(BOX_REF_BUDGET + 1), { refs: 1 }))).rejects.toThrow(/write budget exceeded/)
    })

    test('references must also cover the value being replaced or removed', async () => {
      const { client } = await setup()
      await client.send.put(putParams(tag('k44'), bytes(3000)))

      // A small new value does not make a single reference enough: box_del reads the old box.
      await expect(client.send.put(putParams(tag('k44'), bytes(10), { refs: 1 }))).rejects.toThrow(/read budget exceeded/)
      await expect(client.send.remove(removeParams(tag('k44'), { refs: 1 }))).rejects.toThrow(/read budget exceeded/)
    })
  })
})
